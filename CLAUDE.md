...
2. **푸시는 자동으로 하지 않는다.** 먼저 작업 결과를 보고하고 → **어디에 올릴지(레포·리모트)를 사람이 고른 뒤** → 본인 브랜치로 푸시 → Pull Request. `main` 직접 푸시·`--force` 금지.
...
3. **작업 & 커밋(로컬)** — 본인 담당 파일만, 작게 자주. `git add <변경한 파일만>` → `git commit` (메시지는 3번 형식). 여기까지는 로컬이라 **GitHub엔 올라가지 않는다.**
   - `git add -A` / `git add .` 는 의도치 않은 파일까지 올라간다. **변경한 파일을 명시**해서 add.
4. **보고 & 멈춤 (자동 푸시 금지)** — 푸시하기 전에 **무엇을 했는지 먼저 사람에게 보고**하고 확인을 기다린다.
   - 보여줄 것: `git status` · 변경한 파일 목록 · `git diff --stat`(diff 요약) · 커밋 메시지.
   - **사람이 확인하기 전에는 절대 `git push` 하지 않는다.**
5. **레포 선택 후 푸시** — `git remote -v` 로 **현재 연결된 푸시 대상(레포·리모트)을 보여주고**, 어디에 올릴지 사람에게 고르게 한다.
   - 예: 팀 진실본(`final-team2/...`) vs 개인 포트폴리오(`jsc1209/...`) — 어디로 올릴지(또는 둘 다인지) 확인.
   - 사람이 고른 **그 리모트·그 브랜치에만** 푸시: `git push <고른_리모트> feat/...`
6. **PR** — 푸시한 레포에서 Pull Request 생성. **한 PR = 한 기능/한 관심사.** 담당 영역이 겹치면 리뷰 후 머지.
...
[끝나면] 변경한 파일 목록과 diff 요약(git diff --stat)만 보여줘. git push 하지 마 — 내가 확인하고, 어느 레포에 올릴지 직접 정해서 푸시한다.
